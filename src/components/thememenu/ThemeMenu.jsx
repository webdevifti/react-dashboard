...
const ThemeMenu = () => {
    ...
    const setActiveMenu = () => menu_ref.current.classList.add('active-thme')
    const closeMenu = () => menu_ref.current.classList.remove('active-thme')
                    ...
                    <ul className="mode-list">
                        {
                            mode_settings.map((item, index) => (
                                <li key={index} onClick={() => setMode(item)}>
                                    <div className={`mode-list-color ${item.background} ${item.id === currentMode ? 'active-color' : '' } `}>
                    ...
                    <ul className="mode-list">
                        {
                            color_setting.map((item, index) => (
                                <li key={index} onClick={() => setColor(item)}>
                                    <div className={`mode-list-color ${item.background} ${item.id === currentColor ? 'active-color' : '' }`}>
                                        <i className="bx bx-check"></i>
                                    </div>
                                    <span>{item.name}</span>
                                </li>
